/*
My API

This is the API

API version: 1.0.0
*/

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package main

import (
	"encoding/json"
	"fmt"
)

// checks if the NFSConfig type satisfies the MappedNullable interface at compile time
var _ MappedNullable = &NFSConfig{}

// NFSConfig struct for NFSConfig
type NFSConfig struct {
	Type string `json:"type"`
	RootUrl string `json:"rootUrl"`
}

type _NFSConfig NFSConfig

// NewNFSConfig instantiates a new NFSConfig object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed
func NewNFSConfig(type_ string, rootUrl string) *NFSConfig {
	this := NFSConfig{}
	this.Type = type_
	this.RootUrl = rootUrl
	return &this
}

// NewNFSConfigWithDefaults instantiates a new NFSConfig object
// This constructor will only assign default values to properties that have it defined,
// but it doesn't guarantee that properties required by API are set
func NewNFSConfigWithDefaults() *NFSConfig {
	this := NFSConfig{}
	return &this
}

// GetType returns the Type field value
func (o *NFSConfig) GetType() string {
	if o == nil {
		var ret string
		return ret
	}

	return o.Type
}

// GetTypeOk returns a tuple with the Type field value
// and a boolean to check if the value has been set.
func (o *NFSConfig) GetTypeOk() (*string, bool) {
	if o == nil {
		return nil, false
	}
	return &o.Type, true
}

// SetType sets field value
func (o *NFSConfig) SetType(v string) {
	o.Type = v
}

// GetRootUrl returns the RootUrl field value
func (o *NFSConfig) GetRootUrl() string {
	if o == nil {
		var ret string
		return ret
	}

	return o.RootUrl
}

// GetRootUrlOk returns a tuple with the RootUrl field value
// and a boolean to check if the value has been set.
func (o *NFSConfig) GetRootUrlOk() (*string, bool) {
	if o == nil {
		return nil, false
	}
	return &o.RootUrl, true
}

// SetRootUrl sets field value
func (o *NFSConfig) SetRootUrl(v string) {
	o.RootUrl = v
}

func (o NFSConfig) MarshalJSON() ([]byte, error) {
	toSerialize,err := o.ToMap()
	if err != nil {
		return []byte{}, err
	}
	return json.Marshal(toSerialize)
}

func (o NFSConfig) ToMap() (map[string]interface{}, error) {
	toSerialize := map[string]interface{}{}
	toSerialize["type"] = o.Type
	toSerialize["rootUrl"] = o.RootUrl
	return toSerialize, nil
}

func (o *NFSConfig) UnmarshalJSON(bytes []byte) (err error) {
    // This validates that all required properties are included in the JSON object
	// by unmarshalling the object into a generic map with string keys and checking
	// that every required field exists as a key in the generic map.
	requiredProperties := []string{
		"type",
		"rootUrl",
	}

	allProperties := make(map[string]interface{})

	err = json.Unmarshal(bytes, &allProperties)

	if err != nil {
		return err;
	}

	for _, requiredProperty := range(requiredProperties) {
		if _, exists := allProperties[requiredProperty]; !exists {
			return fmt.Errorf("no value given for required property %v", requiredProperty)
		}
	}

	varNFSConfig := _NFSConfig{}

	err = json.Unmarshal(bytes, &varNFSConfig)

	if err != nil {
		return err
	}

	*o = NFSConfig(varNFSConfig)

	return err
}

type NullableNFSConfig struct {
	value *NFSConfig
	isSet bool
}

func (v NullableNFSConfig) Get() *NFSConfig {
	return v.value
}

func (v *NullableNFSConfig) Set(val *NFSConfig) {
	v.value = val
	v.isSet = true
}

func (v NullableNFSConfig) IsSet() bool {
	return v.isSet
}

func (v *NullableNFSConfig) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullableNFSConfig(val *NFSConfig) *NullableNFSConfig {
	return &NullableNFSConfig{value: val, isSet: true}
}

func (v NullableNFSConfig) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullableNFSConfig) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}


